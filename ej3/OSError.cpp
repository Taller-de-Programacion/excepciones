...
OSError::OSError(const char *fmt, ...) noexcept{
     ...
     */
    va_list args;
    // Inicializamos la lista de argumentos pasandolé como argumento el ultimo argumento que nosotros recibimos
    // fuera de la lista variadica (de forma obligatoria debe existir al menos 1 argumento fijo, que es el formato
    // del mensaje)
    va_start(args, fmt);
